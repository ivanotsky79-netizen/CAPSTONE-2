...
export default function AdminDashboard({ onLogout }) {
    ...
    // Forms
    const [addForm, setAddForm] = useState({ fullName: '', gradeSection: '', lrn: '', studentId: '' });
        ...
        reader.onload = async (evt) => {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (cols.length >= 3) {
                    ...
                    if (name && lrn) data.push({ fullName: name, gradeSection: grade, lrn, studentId: sid });
                }
            }
                            ...
                            <div className="win98-table-wrapper">
                                {usersViewMode === 'all' ? (
                                    ...
                                    <table className="win98-table">
                                        ...
                                        <tbody>
                                            {(() => {
                                                const reqIds = new Set(topupRequests.filter(r => r.status === 'PENDING').map(r => r.studentId));
                                    ...
                                    <tbody>
                                        {topupRequests
                                            .filter(req => {
                                                ...
                                                const isNotResolved = req.status !== 'RESOLVED';
                                                return matchesSearch && matchesDate && matchesSlot && isNotResolved;
                                            })
                                            .length === 0 ? (
